...
def get_estagios(query: Any, ano_de: Any = "", ano_ate: Any = "", nome_do_campus: Any = "", nome_do_centro_unidade: Any = "", nome_do_curso: Any = "") -> list:
    ...
    ⚠️ IMPORTANTE:
    Modifique a variável `query` para remover menções explícitas a nomes de cursos, centros/unidades e campi. Mas matenha o restante da pergunta intacto. Isso é crucial para garantir que a ferramenta funcione corretamente.
    ...
    # if len(estagios_filtrados) > 200:
    #     return f"A quantidade de estágios retornados é muito grande, {len(estagios)}."
    # print("Okay")
